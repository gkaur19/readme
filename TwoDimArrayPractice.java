...
public void printArray( )
{
    ...
    for (int row = 0; row < intArray.length; row++) {
        ...
        for (int column = 0; column < intArray[row].length; column++) {
            ...
            // animate after processing the element
            animate(row, column);
        }
        System.out.println();
    }
...
* @param
value the value to assign to the element of the row
* @param
row the row in which to set the elements to value
*/
public void setValues( int value, int row )
{
    ...
    bc.updateBarChart(2, row, intArray[row].length - 1, this.getGraphics());
}
...
public int findMinimum(int column) {
	 int min = intArray[0][column];
	    ...
	    bc.updateBarChart(3, intArray.length - 1, column, this.getGraphics());
	    return min;
}
...
* @return
the number of times value was found
*/
...
// Write your code here:
    ...
    bc.updateBarChart(4, intArray.length - 1, intArray[0].length - 1, this.getGraphics());
    return count;
...
private class ButtonHandler implements ActionListener {
 public void actionPerformed(ActionEvent e) {
     ...
     if (source == fillValues)
         startActivity(0);
     else if (source == printArray)
         startActivity(1);
     else if (source == setValues)
         startActivity(2);
     else if (source == findMinimum)
         startActivity(3);
     else if (source == countFrequency)
         startActivity(4);

     // Disable buttons while animation runs
     disableButtons();
 }
}
